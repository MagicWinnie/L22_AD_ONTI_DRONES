...
class Recognition:
    ...
    def waitDataColor(self):
        ...
        for _ in range(3):
            ...
            lower_red = np.array([150,70,40])
            upper_red = np.array([180,240,240])
            lower_green = np.array([55,10,10])
            ...
            for i in range(len(res1)):
                for i1 in range(len(res1[0])):
                    ...
                    if res3[i][i1] != 0:
                        yel += 1
            if max(gre, yel, red) == gre:
                self.color = "green"
            elif max(gre, yel, red) == yel:
                self.color = "yellow"
            else:
                self.color = "red"
        ...
        cv2.putText(image, str("Result: " + self.color), (height // 4, width // 4),cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)
        h1, w1 = res1.shape[:2]
        h2, w2 = res2.shape[:2]
...
def navigate_wait(x, y, z, speed=0.4, tolerance=0.18):
...
for i in range(len(d)):
    ...
    # print("d[i]:", d[i], "color:", color, "r2[-1]:", r2[-1])
